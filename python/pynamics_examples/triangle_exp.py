...
for item in numpy.linspace(angle_start,angle_end,num):
    ...
    t_max1 = numpy.append(t_max1,res1.fun)
    t_max2 = numpy.append(t_max2,res2.fun)
...
for item in numpy.linspace(angle_start,angle_end,7):
    ...
    dis_x = item
    dis_y = ft0(item)
    plot_one_config(item,displacement=[dis_x,dis_y*1000-20],amplify=100,side='l') 
    
    error_string1 = "%.7f" % (ft1(item)/0.035)
    plt.text(dis_x,dis_y*1000,error_string1,ha='center',va='top') 
    
    # plot_one_config(item,displacement=[dis_x,dis_y*1000-20],amplify=100,ax=[]) 
    # t_max = numpy.append(t_max,max_T_value)

for item in numpy.linspace(angle_start,angle_end,5):
    initialvalues = {}   
    initialvalues[qF]   =(item)*pi/180
    initialvalues[qF_d] =0*pi/180
    dis_x = item
    dis_y = ft0(item)    
    plot_one_config(item,displacement=[dis_x,dis_y*1000+10],amplify=100,side='r') 
